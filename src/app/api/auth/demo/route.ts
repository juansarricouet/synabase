...
export const POST = withPublic(async () => {
  const result = (await ensureDemoData()) as { created: boolean };
  if (result.created) log.info("demo.seeded", {});
  const user = await findUserByEmail(DEMO_EMAIL as string);
  if (!user) return NextResponse.json({ error: "No pudimos preparar la demo" }, { status: 500 });

  const membership = await one<{ business_id: string }>(
    "SELECT business_id FROM memberships WHERE user_id = $1 ORDER BY created_at ASC LIMIT 1",
    [user.id],
  );
