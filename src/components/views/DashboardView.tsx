...
import {
  ArrowUpRight, ClipboardList, MousePointerClick, QrCode, Repeat, ScanLine,
  Share2, Sparkles, TrendingDown, TrendingUp, UserPlus, Users,
} from "lucide-react";
...
export function DashboardView({
  stats, mainForm, firstName, basePath = "/app",
}: {
    ...
    <div>
      <PageHeader
        ...
        actions={
          mainForm && (
            <Link
              href={`${basePath}/formularios/${mainForm.id}?tab=qr`}
            ...
            <section className="card card-hover flex flex-col p-5">
              ...
              <div className="-mx-2 flex-1 space-y-0.5 overflow-y-auto">
                {stats.recent_submissions.map((s) => (
                  <Link
                    key={s.id}
                    href={s.customer_id ? `${basePath}/clientes/${s.customer_id}` : `${basePath}/base`}
