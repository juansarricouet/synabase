...
import {
  ArrowLeft, CalendarClock, Coffee, Mail, MessageCircle, Phone, Repeat, Target, Wallet,
} from "lucide-react";
import { Badge } from "@/components/ui/Badge";
import { Avatar } from "@/components/ui/misc";
import { formatDate, formatMoney, formatNumber, timeAgo } from "@/lib/utils";
import { CustomerActions, HistoryList, NotesCard, TagsEditor } from "@/app/app/clientes/[id]/profile-widgets";
import type { Segment, Tag } from "@/lib/types";
import type { CustomerFacts, CustomerHistoryEntry } from "@/server/services/customers";

/**
 * Ficha completa del cliente. En modo `readOnly` (demo pública) se ocultan las
 * acciones que escriben y las notas/etiquetas se muestran sin edición.
 */
export function CustomerProfileView({
  customer, history, allTags, segments, basePath = "/app", readOnly = false,
}: {
        ...
        <div className="space-y-4">
          {readOnly ? (
            <>
              <section className="card p-5">
                <h3 className="text-[14px] font-semibold tracking-tight text-ink-950">Notas internas</h3>
              ...
              {customer.tags.length > 0 && (
                <section className="card p-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {customer.tags.map((t) => (
                      <Badge key={t.id} color={t.color as "violet"}>{t.name}</Badge>
                    ))}
                  </div>
                </section>
              )}
          ...
          <section className="card p-5">
            ...
            {segments.length > 0 ? (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {segments.map((s) => (
                  <Link key={s.id} href={`${basePath}/segmentos`}>
